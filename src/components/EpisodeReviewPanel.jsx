...
function VoicePicker({ characterId, name, value, voiceOptions, onChange }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs font-semibold text-slate-300 w-20 truncate">{name}</span>
      <select
        value={isKnown ? value : '__custom'}
        onChange={(e) => onChange(characterId, e.target.value === '__custom' ? '' : e.target.value)}
        className="flex-1 px-2 py-1 text-xs border border-slate-700 rounded-lg outline-none focus:border-reel"
      >
  ...
  return (
    <div className="flex flex-col gap-3 bg-violet-950 ring-1 ring-inset ring-violet-800 rounded-xl p-3">
      ...
      {voiceEntries.length > 0 && (
        <div className="flex flex-col gap-1.5 bg-slate-900 rounded-lg p-2.5 ring-1 ring-inset ring-violet-900">
          <p className="text-[11px] font-bold text-slate-400">{t('episodes.reviewVoicesHeading')}</p>
          ...
          <div key={scene.order} className="bg-slate-900 rounded-lg p-2.5 ring-1 ring-inset ring-violet-900 flex flex-col gap-2">
            ...
            {scene.dialogue.map((line, i) => (
              <div key={i} className="flex flex-col gap-1 pl-2 border-l-2 border-slate-800">
                <div className="flex items-center gap-2">
                  <span className="text-[11px] font-semibold text-slate-400 w-16 truncate">
